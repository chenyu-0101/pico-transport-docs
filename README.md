...
By utilizing PicoTransport, developers can leverage Netcode and PICO room services to develop multiplayer PICO applications. The overall structure is depicted below:    
![add package](images/struct-1.png)

## 2. Preparation
### 2.1. PICO VR Environment Setup
- Follow the["Quick Start"](https://developer-cn.pico-interactive.com/document/unity) section in the PICO official documentation to set up the basic PICO VR environment and create a new VR project.  
...
- Apply for a developer account on the [PICO platform](https://developer-cn.pico-interactive.com/) and create an app. 
- After creating the app, activate the PICO matchmaking and room services for this app:  
...
>To run the included examples, it is essential to create a matchmaking pool named 'test_pool_basic_2' and set the options 'Manage Room' and 'Allow Multiple Matches to This Room' as shown in the image below:    
>![set-matchmaking-pool](images/set-matchmaking-pool.png)  

### 2.3. Unity Project Configuration  
- Configure the 'PICO SDK' and 'PC Debug' settings:  
...
![import-samples](images/import-samples.png) 

### 3.3 Run the SimpleMode Example  
#### 3.3.1. Run in the Editor
- Open the 'SimpleMode' scene at the specified path:  
...
![run-in-editor-simplemode](images/run-in-editor-simplemode.png)  
- Click the Play button to run the scene and add the current scene to the build scenes when prompted 
![build-simplemode-scene](images/build-simplemode-scene.png)  
...
- In the Unity Editor, go to the menu bar and select 'File' > 'Build Settings...'. In the build settings window, remove the basic XR scene added during the preparation step and only check the SampleMode scene: 
![build-simplemode](images/build-simplemode-scene.png)    
- Switch the platform to Android (ensure that the platform settings match the instructions in the [Quick Start](https://developer-cn.pico-interactive.com/document/unity) section of the official PICO documentation).  
...
![build-simplemode-buildrun](images/build-simplemode-buildrun.png)  
...
![matchmakingroom-buildrun](images/matchmakingroom-buildrun.png)  
  - Run it in the Unity Editor, click the 'InitPicoSDK' button, and it will initialize PICO internally and join the 'test_pool_basic_2' matchmaking pool configured in section [2.2](#22-pico-multiplayer-service-activation). After successfully joining the matchmaking pool, the interface will display the UID and wait for other clients to join:  
![matchmakingroom-waiting](images/matchmakingroom-waiting.png)  
  - On the PICO device, click the 'InitPicoSDK' button to join the matchmaking pool. Since the matchmaking conditions are met, the PICO matchmaking service will match the two clients in the Editor with the two clients on the PICO device and create a matchmaking room for them to join. At this point, the interface will display the 'JoinRoom' button:    
![matchmakingroom-joinroom](images/matchmakingroom-joinroom.png)  
  - Click the JoinRoom button in both the Editor and on the PICO device to join the PICO matchmaking room. Once in the room, the position synchronization for NetCode will start, as shown in the following image. Click the 'Submit position' button to move the capsule representing yourself and synchronize the positions between the two clients:  
![matchmakingroom-inroom](images/matchmakingroom-inroom.png)  
>Note:   
>1. PICO's matchmaking rooms have a time limit. If you don't click 'JoinRoom' to join the room, the room will be destroyed on the PICO server side after 10 seconds. After that, any attempt to join the room will fail, and you will be returned to the initial interface.  
...
![multiplayer-inmatchmaking](images/multiplayer-inmatchmaking.png)  
- On the PICO device, click the 'Start' button to join the matchmaking. Since the matchmaking conditions are met, the PICO matchmaking service will match the client in the Editor with the client on the PICO device and create a matchmaking room. Once both clients enter this matchmaking room, the position synchronization for NetCode will start. Move to the green area on the right to exit the current scene:  
...
- When the host leaves in NetCode, the entire game session ends, and the client will also exit the game session.   
...
#### 3.5.3 Behavior in Example Scenarios with these Factors:  
...
- If the host leaves the room, and within 10 seconds the PICO server still keeps the room, the host can rejoin the room.  
...
- In case of a network disconnection:  
  ...
  - If a client fails to reconnect within 10 seconds or keeps failing to reconnect:  
    ...
    - The other client with a stable network connection will see the disconnected client leaving the scene if they were the host, or they will exit themselves if the disconnected client was the host. 
...
The logic after entering the room, which is related to synchronization, needs to be supported by NetCode. In the stage of logic inside the room, PicoTransport acts as the communication layer for NetCode, responsible for supporting the low-level message communication of NetCode.  
PicoTransport has two working modes, 'SimpleMode' and 'ExternalRoom' mode, which are related to these two parts of the logic:    
- In 'SimpleMode', there is no logic outside the room. The developer only needs to set a common room name, and PicoTransport will handle the rest internally, including gathering suitable players together and joining them in the same PICO named room. In this mode, the PICO room is transparent to the developer.    
...
![import-samples](images/import-samples.png)  
- Set the transport of the NetworkManager to PicoTransport.  
![use-transport-component](images/use-transport-component.png)  
...
### 4.3 Driving PicoTransport in ExternalRoom Mode  
  ...
  After joining the PICO room, the client calls StartHost or StartClient to start Netcode. During the operation of Netcode, developers need to do two things in their code:  
  - Forward the received room change information and messages to PicoTransport.  
>Events include: player enter and leave PICO room events, as well as the room player list.   
>Messages include: Netcode messages from players in the same room.    
  - When PicoTransport needs to send a message, call the PICO room service interface to send the message to the specified target player.      
![seq-external-mod](images/seq-external-mod.png)    
...
|---------|-----------|-----|
| Init    | Initialize PicoTransport | After finding suitable other players and successfully joining the same room.<br>For example, in the OnComplete of PICO SDK's RoomService.Join2. |  
| HandleRoomInfoUpdate | Notify PicoTransport of the changes in the PICO room information | When the room information changes, such as when someone enters the room.<br>For example, called in the callback set by RoomService.SetUpdateNotificationCallback.|  
| Update | Drive the process periodically | Called every frame.<br>Inside this function, HandleMsgFromRoom is called to receive messages from the PICO room and pass them to PicoTransport. | 
...
>This class encapsulates the initialization of the PICO platform SDK, token retrieval, PICO User retrieval, PICO Game service initialization, PICO matchmaking, entering the room, and other related interfaces. It also registers callbacks to handle messages and events from the PICO services.  
...
>|---------|-----------|
>| StartPicoGame | nitialize the PICO platform, retrieve the access token, retrieve the logged-in user, and call game initialize. |
>| StartMatchmaking/StartJoinRoom/StartLeaveRoom | Join matchmaking, join a room, leave a room. |
...
- If the host leaves the PICO room, the current Netcode session of all clients should end. However, in PICO, clients other than the original host remain in the PICO room without exiting. At this point, developers have two options:
  ...
  - Let the new PICO room host restart StartHost, and other regular members in the room restart StartClient. This way, Netcode can be started again and the original game can continue. To do this, developers need to handle the host leaving event appropriately and save and restore the game state. PicoTransport provides some support for this by calling StartHost/StartClient again depending on whether it is the new PICO room host or not. Please refer to the Multiplayer code for more details. 
- PicoTransport internally handles the sending and receiving of messages within the PICO room. The business logic should not use the message sending and receiving functions in the PICO SDK's NetworkService.  
